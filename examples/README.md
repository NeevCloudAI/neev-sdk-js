...
| [`ssh-tunnel.ts`](./ssh-tunnel.ts) | `sandbox.ssh()` — open a local SSH tunnel and run a command over it with the system `ssh` (needs `pnpm add -D ws`) | `npx tsx examples/ssh-tunnel.ts` |
